...
const TABS = ['Overview', 'Inpatient', 'Readmissions', 'OP / ED', 'SNF', 'Pharmacy'];
...
export function FinancialView({ showToast }) {
  ...
  return (
    <>
      ...
      {tab === 0 && <OverviewTab data={costByProvider} />}
      {tab === 1 && <InpatientTab data={ipCostDetail} />}
      {tab === 2 && <ReadmissionsTab bars={readmissionRates} />}
      {tab === 3 && <OpEdTab data={opEdUtil} />}
      {tab === 4 && <SnfTab data={snfSpending} />}
...
function OpEdTab({ data }) {
  const rows = safeTableRows(data, (FALLBACK_TABLES.op_ed_utilization || {}).rows);
  return (
